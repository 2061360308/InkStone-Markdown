...
class Fs {
  ...
  get = async (path: string, repo: string) => {
    ...
    //   .first();
    const file = await this.db.table('files').where({ path, repo }).first()
    return file.content
  }
  ...
  list = async (repo: string): Promise<string[]> => {
    // 获取所有文件
    const files = await this.db.table('files').where({ repo }).toArray()
    if (!files) {
      return []
    }
  ...
  listAllRepoNames = async (): Promise<string[]> => {
    // 获取所有仓库名
    const files = await this.db.table('files').toArray()
    const repoNames = files.map((file) => file.repo)
    // 去重
    const result = Array.from(new Set(repoNames))
    return result
  }
}
