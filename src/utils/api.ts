...
export const checkBranch = async (owner: string, token: string, repo: string, branch: string) => {
  // 检查分支是否存在
  const octokit = new Octokit({
    auth: token,
  })
...
class GithubApi {
  ...
  init = async (repo: string, token: string, branch: string) => {
    ...
    const { repoValid, hasPushAccess } = await checkRepo(this.owner as string, repo, token)
    ...
    // 检查分支是否存在
    result.branchValid = await checkBranch(this.owner as string, token, repo, branch)
  ...
  getFileContent = async (path: string, branch = '') => {
    ...
    try {
      ...
      const response: RestEndpointMethodTypes['repos']['getContent']['response'] = await (
  ...
  getFileLastCommitTime = async (path: string, branch = '') => {
    ...
    try {
      ...
      const response: RestEndpointMethodTypes['repos']['listCommits']['response'] = await (
        this.octokit as Octokit
      ).repos.listCommits({
        owner: this.owner as string,
        repo: this.repo as string,
  ...
  commitChanges = async (
    ...
    try {
      ...
      // 获取最新提交的树对象
      const { data: commitData } = await (this.octokit as Octokit).git.getCommit({
        owner: this.owner as string,
        repo: this.repo as string,
      ...
      // 创建新的树对象
      const tree: RestEndpointMethodTypes['git']['createTree']['parameters']['tree'] = files.map(
        (file) => {
      ...
      // 创建新的提交对象
      const { data: newCommitData } = await (this.octokit as Octokit).git.createCommit({
        owner: this.owner as string,
        repo: this.repo as string,
        message,
        tree: treeData.sha,
  ...
  private findGist = async (): Promise<string | null> => {
    ...
    try {
      ...
      const gist = response.data.find((gist) => gist.description === this.description)
